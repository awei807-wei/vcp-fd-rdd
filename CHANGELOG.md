...
- fast scan bootstrap 不再每个 tick 通过 diagnostics 路径全量物化 cold snapshot：冷段目录候选收集改为流式早停，调度层在 L0 + sentinel 候选池饱和或候选耗尽时跳过/冷却 bootstrap，避免 80 万级 manifest-only base 被每秒枚举造成 CPU 常驻和 RSS 高水位。
