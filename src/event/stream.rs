...
use crate::event::watcher::{EventWatcher, watch_roots};
...
impl EventPipeline {
    ...
    pub async fn start(&self) -> anyhow::Result<()> {
        ...
        let (mut rx, mut watcher) = EventWatcher::start(&roots, self.channel_size, overflow_drops.clone())?;
        ...
        tokio::spawn(async move {
            ...
            let mut last_rebuild_at = tokio::time::Instant::now()
...
fn merge_events(seq: &mut u64, raw: Vec<notify::Event>) -> Vec<EventRecord> {
    ...
    for ev in raw {
        // 处理 Rename（双路径事件）
        if matches!(ev.kind, notify::EventKind::Modify(notify::event::ModifyKind::Name(_)))
            && ev.paths.len() >= 2
        {
            ...
            *seq += 1;
            merged.insert(to.clone(), EventRecord {
                seq: *seq,
                timestamp: now,
        ...
        // 合并策略：后到的事件覆盖先到的
        *seq += 1;
        merged.insert(path.clone(), EventRecord {
            seq: *seq,
            timestamp: now,
            event_type,
            path,
