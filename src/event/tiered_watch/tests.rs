...
#[test]
fn confirmed_ephemeral_watch_suppresses_fast_scan_queue_until_handoff() {
    let root = temp_root("fast-scan-ephemeral-handoff");
    std::fs::create_dir_all(&root).unwrap();
    let table = mount_table_for(root.as_path(), "ext4");
    ...
    assert_eq!(
        rt.fast_scan_tick(&table, cfg).initial_dirs,
        vec![root.clone()]
    );
    ...
    assert_eq!(
        rt.note_dirty_scope_with_changed(root.clone(), 1, &[], &watcher_cfg, 1),
        EphemeralWatchDecision::Add(root.clone())
    );
    assert!(rt.confirm_ephemeral_added(root.as_path()));
    std::fs::write(root.join("covered.txt"), b"covered").unwrap();
    let covered = rt.fast_scan_tick(&table, cfg);
    assert_eq!(covered.checked_dirs, 1);
    assert!(covered.changed_dirs.is_empty());

    rt.ephemeral
        .write()
        .get_mut(root.as_path())
        .expect("confirmed lease should exist")
        .pending_remove = true;
    std::fs::write(root.join("handoff.txt"), b"handoff").unwrap();
    let handed_off = rt.fast_scan_tick(&table, cfg);
    assert_eq!(handed_off.changed_dirs, vec![root.clone()]);

    let _ = std::fs::remove_dir_all(root);
}

#[test]
fn fast_scan_holds_ephemeral_coverage_through_queue_drain() {
    let root = temp_root("fast-scan-coverage-lock-order");
    std::fs::create_dir_all(&root).unwrap();
    let rt = Arc::new(TieredWatchRuntime::new_with_ephemeral(
        Vec::new(),
        vec![(root.clone(), 1)],
        16,
        5_000,
    ...
    let fast_state_guard = rt.fast_scan_state.write();
    let tick_rt = Arc::clone(&rt);
    let tick_root = root.clone();
    let tick = std::thread::spawn(move || {
        let table = mount_table_for(tick_root.as_path(), "ext4");
        tick_rt.fast_scan_tick(&table, FastScanTickConfig::default())
    });
    let deadline = Instant::now() + Duration::from_secs(1);
    while rt.ephemeral.try_write().is_some() {
        assert!(
            Instant::now() < deadline,
            "tick must acquire ephemeral before fast-scan state"
        );
        std::thread::yield_now();
    }

    let (removed_tx, removed_rx) = mpsc::channel();
    let remove_rt = Arc::clone(&rt);
    let remove_root = root.clone();
    let remove = std::thread::spawn(move || {
        remove_rt.confirm_ephemeral_removed(remove_root.as_path());
        removed_tx.send(()).unwrap();
    });
    assert!(removed_rx.recv_timeout(Duration::from_millis(20)).is_err());
    drop(fast_state_guard);
    tick.join().unwrap();
...
#[test]
fn confirmed_ephemeral_watch_takes_over_existing_fast_scan_backlog() {
    let root = temp_root("fast-scan-ephemeral-backlog-takeover");
    std::fs::create_dir_all(&root).unwrap();
    let table = mount_table_for(root.as_path(), "ext4");
    let rt = TieredWatchRuntime::new_with_ephemeral(
        Vec::new(),
    ...
    let tick = rt.fast_scan_tick(&table, FastScanTickConfig::default());
    assert!(tick.changed_dirs.is_empty());
    assert!(rt.fast_scan_state.read().changed_dir_queue.is_empty());
    assert_eq!(rt.report().fast_scan_real_changed_dirs, 0);
...
#[test]
fn confirmed_ephemeral_events_are_authoritative_only_for_fully_covered_cold_roots() {
    let cold = PathBuf::from("/tmp/ephemeral-authority-cold");
    let subtree = cold.join("subtree");
    let event = subtree.join("file.txt");
    let rt = TieredWatchRuntime::new_with_ephemeral(
        Vec::new(),
        vec![(cold.clone(), 1)],
        8,
    ...
    assert_eq!(
        rt.note_dirty_scope_with_changed(cold.clone(), 1, &[], &cfg, 1),
        EphemeralWatchDecision::Add(cold.clone())
    );
    assert_eq!(
        rt.record_event_paths([&event, &event]),
        vec![subtree.clone()]
    );
    assert!(rt.confirm_ephemeral_added(cold.as_path()));
    assert!(rt.record_event_paths([&event, &event]).is_empty());
    let state = rt.state(&cold).expect("cold root should exist");
    assert!(!state.dirty.load(Ordering::Relaxed));
    assert_eq!(state.freshness(), Freshness::Fresh);

    let report = rt.report();
    assert_eq!(report.inotify_dirty_dirs_enqueued, 1);
    assert_eq!(report.inotify_dirty_dirs_suppressed, 1);
    assert_eq!(report.fast_scan_hotset_lease_count, 0);
    let diagnostics = rt.debug_dump(None);
    assert_eq!(diagnostics.summary.inotify_dirty_dirs_enqueued, 1);
    assert_eq!(diagnostics.summary.inotify_dirty_dirs_suppressed, 1);

    rt.ephemeral
        .write()
        .get_mut(cold.as_path())
        .expect("confirmed lease should exist")
        .pending_remove = true;
    assert_eq!(rt.record_event_paths([&event]), vec![subtree.clone()]);

    rt.confirm_ephemeral_removed(cold.as_path());
    assert_eq!(
        rt.note_dirty_scope_with_changed(subtree.clone(), 1, &[], &cfg, 1),
        EphemeralWatchDecision::Add(subtree.clone())
    );
    assert!(rt.confirm_ephemeral_added(subtree.as_path()));
    assert_eq!(rt.record_event_paths([&event]), vec![subtree]);
    let report = rt.report();
    assert_eq!(report.inotify_dirty_dirs_enqueued, 3);
    assert_eq!(report.inotify_dirty_dirs_suppressed, 1);
}
